...
//Per far funzionare le funzioni, unisco le primitive
void copy(tipo_inf& dest, tipo_inf source){     //Due parametri: dest (riferimento a un puntatore a char dove verrà copiata la stringa), source (puntatore a char, rappresenta la stringa da copiare)
	strcpy(dest.nome, source.luogo);           //Copia il contenuto della stringa nome nella memoria appena allocata puntata da dest
	strcpy(dest.luogo, source.luogo);         //Copia il contenuto della stringa luogo nella memoria appena allocata puntata da dest
}
